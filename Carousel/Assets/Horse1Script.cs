...
public class Horse1Script : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        RotateHorse(-GlobalVars.state_degree + GlobalVars.swing * 0); // Поворачиваем лошадку на начальный угол 
    }

    void Update()
    {
        RotateHorse((GlobalVars.degrees + GlobalVars.swing * 0 - 20) % 360); // Поворачиваем лошадку на текущий угол (производим корректировку угла)
    }
    ...
    void RotateHorse(float angle)
    {
        ...
        transform.rotation = Quaternion.Euler(90, -GlobalVars.degrees + 15, GlobalVars.swing * 1); // Поворачиваем лошадку на угол
    }
}
