...
public class Horse4Script : MonoBehaviour
{
    float posy; // Переменная хранящая позицию по "у"
    void Start()
    {
        posy = transform.position.y; // Запоминаем начальное положение по "у"
        RotateHorse(-GlobalVars.state_degree + GlobalVars.swing * 3); // Поворачиваем лошадку на начальный угол
    }

    void Update()
    {
        RotateHorse((GlobalVars.degrees + GlobalVars.swing * 3 - 10) % 360); // Поворачиваем лошадку на текущий угол (производим корректировку угла)
    }
    ...
    void RotateHorse(float angle)
    {
        ...
        transform.rotation = Quaternion.Euler(90, -GlobalVars.degrees + 10, GlobalVars.swing * 0); // Поворачиваем лошадку на угол
    }
}
